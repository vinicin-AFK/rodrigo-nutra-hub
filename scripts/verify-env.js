 ...
 * 
 * Valida que as variáveis do Supabase estão corretas antes do build
 */
...
function validateEnv() {
  ...
  if (hasErrors) {
    console.error('❌ VALIDAÇÃO FALHOU - Corrija os erros antes de continuar');
    console.error('\n📋 Configure no .env.local:');
    console.error(`   VITE_SUPABASE_URL=${SUPABASE_URL_REQUIRED}`);
    console.error('   VITE_SUPABASE_ANON_KEY=sua_chave_aqui');
    process.exit(1);
  } else {
    console.log('✅ VALIDAÇÃO PASSOU - Todas as variáveis estão corretas');
    console.log('🔍 ============================================\n');
    process.exit(0);
  }
}
